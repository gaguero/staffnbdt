...
import { organizationService, Organization, UpdateOrganizationData } from '../services/organizationService';
...
const EditOrganizationModal: React.FC<EditOrganizationModalProps> = ({
    ...
    if (organization) {
      setFormData({
        ...
        settings: {
          ...
          supportedLanguages: organization.settings?.supportedLanguages || ['en'],
          theme: organization.settings?.theme || 'default',
          additional: organization.settings?.additional || {},
        },
        branding: {
          primaryColor: organization.branding?.primaryColor || '#AA8E67',
          secondaryColor: organization.branding?.secondaryColor || '#F5EBD7',
          accentColor: organization.branding?.accentColor || '#4A4A4A',
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name.includes('.')) {
      ...
      setFormData(prev => ({
        ...prev,
        [parent]: {
          ...(prev[parent as keyof UpdateOrganizationData] as any),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Clean up form data - only send changed fields
      const submitData: UpdateOrganizationData = {};

      // Check each field for changes
      if (formData.name?.trim() !== organization.name) {
        submitData.name = formData.name.trim();
      }
      
      if (formData.slug?.trim() !== organization.slug) {
      ...
      if (formData.timezone?.trim() !== (organization.timezone || '')) {
        submitData.timezone = formData.timezone?.trim();
      }
      
      if (formData.website?.trim() !== (organization.website || '')) {
        submitData.website = formData.website?.trim();
      }
      
      if (formData.contactEmail?.trim() !== (organization.contactEmail || '')) {
        submitData.contactEmail = formData.contactEmail?.trim();
      }
      
      if (formData.contactPhone?.trim() !== (organization.contactPhone || '')) {
        submitData.contactPhone = formData.contactPhone?.trim();
      }
      
      if (formData.isActive !== organization.isActive) {
        submitData.isActive = formData.isActive;
      }

      // Check settings changes
      const settingsChanged = 
        formData.settings?.defaultLanguage !== organization.settings?.defaultLanguage ||
        JSON.stringify(formData.settings?.supportedLanguages?.sort()) !== JSON.stringify(organization.settings?.supportedLanguages?.sort()) ||
        formData.settings?.theme !== organization.settings?.theme;
      ...
      const brandingChanged = 
        formData.branding?.primaryColor !== organization.branding?.primaryColor ||
        formData.branding?.secondaryColor !== organization.branding?.secondaryColor ||
        formData.branding?.accentColor !== organization.branding?.accentColor ||
        formData.branding?.logoUrl !== organization.branding?.logoUrl ||
        formData.branding?.faviconUrl !== organization.branding?.faviconUrl;
