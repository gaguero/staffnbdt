...
import { ConciergeObject, UpdateConciergeObjectInput, ObjectFieldDefinition, ObjectType } from '../../types/concierge';
...
const EditConciergeObjectModal: React.FC<EditConciergeObjectModalProps> = ({
  isOpen,
  onClose,
  object,
  onSuccess,
  ...
  const [formData, setFormData] = useState<UpdateConciergeObjectInput>({
    type: object.type,
  ...
  // Initialize form data when object changes
  useEffect(() => {
    if (object) {
      // Convert attributes array to object for form handling
      const attributesObj: Record<string, any> = {};
      object.attributes.forEach(attr => {
        attributesObj[attr.fieldKey] = 
          attr.stringValue || 
          attr.numberValue || 
          attr.booleanValue || 
          attr.dateValue || 
  ...
  const loadObjectTypes = async () => {
    try {
      ...
      // Find and set the selected type based on current object
      const currentType = response.data.find(ot => ot.id === object.type);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updateData: UpdateConciergeObjectInput = {};
      
      if (formData.type !== object.type) updateData.type = formData.type;
      if (formData.reservationId !== object.reservationId) updateData.reservationId = formData.reservationId;
      if (formData.guestId !== object.guestId) updateData.guestId = formData.guestId;
      if (formData.status !== object.status) updateData.status = formData.status;
      if (formData.dueAt !== object.dueAt) updateData.dueAt = formData.dueAt;
      
      // Check if attributes have changed
      const currentAttributesObj: Record<string, any> = {};
      object.attributes.forEach(attr => {
        currentAttributesObj[attr.fieldKey] = 
          attr.stringValue || 
          attr.numberValue || 
          attr.booleanValue || 
          attr.dateValue || 
      ...
      if (JSON.stringify(formData.attributes) !== JSON.stringify(currentAttributesObj)) {
        updateData.attributes = formData.attributes;
      }
      ...
      await conciergeService.updateObject(object.id, updateData);
      toastService.actions.updated('Concierge Object');
      onSuccess();
    } catch (error: any) {
      console.error('Failed to update concierge object:', error);
      setError(error.response?.data?.message || error.message || 'Failed to update concierge object');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-semibold text-charcoal">
              Edit Concierge Object: {object.id.slice(-8)}
            </h3>
            <button
              onClick={handleClose}
...
export default EditConciergeObjectModal;